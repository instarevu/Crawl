...
import com.ir.core.crawllib.util.IO;
...
public class CrawlController extends Configurable {
	...
	/**
	 * Is the crawling session set to 'shutdown'. ItemCrawler threads monitor this
	...
	protected <T extends WebCrawler> void start(final Class<T> _c, final int numberOfCrawlers, boolean isBlocking, String clusterId) {
		try {
			...
			for (int i = 1; i <= numberOfCrawlers; i++) {
				...
				threads.add(thread);
				logger.info("ItemCrawler " + i + " started.");
			}
								...
								for (int i = 0; i < threads.size(); i++) {
									...
									if (!thread.isAlive()) {
										if (!shuttingDown) {
											...
											T crawler = _c.newInstance();
											thread = new Thread(crawler, "ItemCrawler " + (i + 1));
											threads.remove(i);
	...
	/**
	 * Set the current crawling session set to 'shutdown'. ItemCrawler threads
